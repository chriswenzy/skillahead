...
export default function AcademicBoard() {
  return (
    <section className="py-32 bg-white">
      <div className="max-w-[1440px] mx-auto px-8 md:px-16 mb-20 flex flex-col md:row justify-between items-end gap-8">
        <div>
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-4 block">Leadership</span>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">The Academic Board</h2>
        ...
        <Link href="#" className="font-bold text-sm text-primary flex items-center gap-2 hover:gap-4 transition-all pb-1 border-b border-primary/20">
      ...
      <div className="max-w-[1440px] mx-auto px-8 md:px-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {boardMembers.map((member, index) => (
          <div key={index} className="group cursor-pointer">
            <div className="relative aspect-[3/4] bg-slate-100 rounded-xl overflow-hidden mb-6 grayscale group-hover:grayscale-0 transition-all duration-700">
            ...
            <p className="text-xs uppercase font-bold tracking-widest text-slate-400 mt-2">{member.role}</p>
          </div>
        ))}
      </div>
    </section>
