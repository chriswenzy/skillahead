        ...
        <div className="col-span-12 lg:col-span-6 order-2 lg:order-1 mt-8 lg:mt-0">
           <div className="grid grid-cols-1 xs:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className={`${item.bg} p-6 md:p-8 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 hover:-translate-y-1 transition-transform cursor-default`}>
                   <span className="text-2xl md:text-3xl">{item.icon}</span>
                   <span className="text-[10px] md:text-xs font-black text-slate-900 uppercase tracking-widest">{item.title}</span>
                </div>
              ))}
           </div>
        </div>
        
        <div className="col-span-12 lg:col-span-6 order-1 lg:order-2 space-y-6 md:space-y-8">
           <span className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">Audience</span>
           <h2 className="text-3xl md:text-5xl font-black text-slate-900 leading-[1.1] tracking-tight">Who This Is For</h2>
